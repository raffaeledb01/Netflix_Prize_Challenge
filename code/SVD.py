...
def SVD(df):
    ...
    print('Creazione dataset per SVD')
    # get just top 100K rows for faster run time
    data = Dataset.load_from_df(df[['Cust_Id', 'Movie_Id', 'Rating']][:1000000], reader)

    # Configura i parametri per GridSearchCV con il modello SVD
    param_grid_svd = {
        'n_factors': [89,100,120], # inizialmente 80,100,120
        'n_epochs': [10,20,30], # inizialmente 10,20,30
        'lr_all': [0.003,.005, 0.007], # inizialmente 0.003, 0.005, 0.007
        'reg_all': [0.2, 0.1] # 0.2, 0.1
    }
    ...
    print("Best parameters for SVD (RMSE):", gs_svd.best_params['rmse'])
    print("Best MAE score for SVD:", gs_svd.best_score['mae'])
    print("Best parameters for SVD (MAE):", gs_svd.best_params['mae'])
    ...
    results_df.to_csv('results_SVD.csv', index=False)
    ...
    try:
        ...
        train_rmse = []
        algo = SVD(n_factors=gs_svd.best_params['rmse']['n_factors'], 
    ...
    try:
        ...
        print(f"Errore durante l'addestramento e la previsione del miglior modello SVD: {e}")

    print("Calcolo delle stime dei rating previsti...")

    print("Creazione del grafico dei Valori Singolari...")
    try:
        ...
        plt.savefig('plots/valori_singolari.png')
        plt.close()
    except Exception as e:
        print(f"Errore durante la creazione del grafico dei valori singolari: {e}")
    ...
    def plot_rmse(rmse, algorithm):
        ...
        plt.savefig('./plots_SVD/rmse_vs_epochs.png')
        plt.close()
